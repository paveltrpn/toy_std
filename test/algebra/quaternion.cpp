...
TEST(quaterniond, construct) {
    ...
    EXPECT_EQ(second[1], 2.0);
    EXPECT_EQ(second[2], 3.0);
    EXPECT_EQ(second[3], 4.0);

    toy::algebra::quaterniond fromQuat{ second };

    EXPECT_EQ(fromQuat[0], 1.0);
    EXPECT_EQ(fromQuat[1], 2.0);
    EXPECT_EQ(fromQuat[2], 3.0);
    EXPECT_EQ(fromQuat[3], 4.0);

    std::array<double, 4> tstArr{ 1.0, 2.0, 3.0, 4.0 };
    toy::algebra::quaterniond fromArray{ tstArr };

    EXPECT_EQ(fromArray.x(), 1.0);
    EXPECT_EQ(fromArray.y(), 2.0);
    EXPECT_EQ(fromArray.z(), 3.0);
    EXPECT_EQ(fromArray.w(), 4.0);
}
