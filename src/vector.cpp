...
export {
    ...
    struct vector_iterator {
        ...
        vector_iterator( const self& rhs )
        ...
        reference operator*() { return range_[element_]; };

        reference operator*() const { return range_[element_]; };

        pointer operator->() { return &range_[element_]; }

        pointer operator->() const { return &range_[element_]; }
        ...
        reference operator[]( size_type element ) { return range_[element]; }
    ...
    struct vector_const_iterator {
        ...
        const value_type& operator*() { return range_[element_]; };

        const value_type& operator*() const { return range_[element_]; };

        const pointer operator->() { return &range_[element_]; }

        const pointer operator->() const { return &range_[element_]; }
    ...
    struct vector {
        ...
        /*
             ...
             */
        vector( size_t s, value_type value )
            : size_{ s } {
            if ( s >= 1 ) {
                capacity_ = size_;
                data_ = std::make_unique<value_type[]>( capacity_ );
...
#ifdef DEBUG
            std::cout << std::format(
                "WARNING! vector(size_t, T): with zero size!\n" );
#endif
        }
        ...
        // element access
        value_type& operator[]( size_t id ) { return data_[id]; }

        const value_type& operator[]( size_t id ) const { return data_[id]; }
    ...
    private:
        constexpr size_t __expand( size_t capacity ) { return capacity * 2; }
